...
class SiteController extends Controller
{
    ...
    public function showSeat($id){
        ...
        $backDate = Session::get('back_date'); 
        $pickup= Session::get('pickup');
        $destination =Session::get('destination');
        return view($this->activeTemplate.'book_ticket', compact('pageTitle','trip' , 'stoppages','busLayout', 'layout','backDate','pickup','destination'));
    }
    ...
    public function bookTicket(Request $request,$id){
        ...
        $getPrice    = $ticketPrice->prices()
                ...
                // calculate sub_total
                $subTotal= 0;
                $ticketPrice = TicketPrice::where(['fleet_type_id'=>$trip->fleet_type_id,'vehicle_route_id'=>$trip->vehicle_route_id])->first();

                foreach($seats as $seat){
                        $parts = explode('-', $seat);
                        $secondPart = $parts[1];       
                        $seatPrice = Seat::where(['ticket_price_id' => $ticketPrice->id,
                        'name'=>$secondPart])->pluck('price')->first();
                        $subTotal += $seatPrice;
                }
                // $unitPrice = getAmount($getPrice->price);
                $pnr_number = getTrx(10);
                if($request->back_date == null){
                ...
                $bookedTicket->sub_total = $subTotal;
                $bookedTicket->save();
                session()->put('pnr_number',$pnr_number);
                return redirect()->route('user.deposit');
        }
        else{
            $pickup_point = session('pickup_point_go',$request->pickup_point);
            $dropping_point = session('dropping_point_go',$request->dropping_point);
            $date_of_journey = session('date_of_journey_go',$request->date_of_journey);
            $back_date_value= session('back_date_value',$request->back_date);
            $gender = session('gender_go',$request->gender);
            $seats[] = session(['seats_go' => $request->seats]);
            $trip_go = session('trip',$id);
            $trip_go_find = Trip::findOrFail($id);
            $trip_back = Trip::with( ['fleetType' ,'route', 'schedule', 'startFrom' , 'endTo', 'assignedVehicle.vehicle', 'bookedTickets'])
            ->where('status', 1)
            ->where(['start_from'=>$trip_go_find->end_to,'end_to'=>$trip_go_find->start_from])
            ->firstOrFail();
            $pageTitle = $trip_back->title;
            $route     = $trip_back->route;
            $stoppageArr = $trip_back->route->stoppages;
            $stoppages = Counter::routeStoppages($stoppageArr);
            $busLayout = new BusLayout($trip_back);
            if(auth()->user()){
                $layout = 'layouts.master';
            }else{
                $layout = 'layouts.frontend';
            }
            return view($this->activeTemplate.'book_ticket_back',compact('back_date_value','pickup_point','dropping_point','date_of_journey','gender','seats','trip_go','trip_back','pageTitle',
            'route',
            'stoppageArr',
            'stoppages',
            'busLayout','layout','trip_go_find'));
        }
    }
    public function bookTicketBack(Request $request ,$id)
    {
        $request->validate([
            "seats"           => "required|string",
        ],[
        ...
        $back_date  = Carbon::parse($request->date_of_journey);
        $today            = Carbon::today()->format('Y-m-d');
        if($back_date->format('Y-m-d') < $today ){
            $notify[] = ['error', 'Date of journey cant\'t be less than today.'];
            return redirect()->back()->withNotify($notify);
        }
        $dayOff =  $back_date->format('w');
        $trip   = Trip::findOrFail($id);
        $route              = $trip->route;
        $stoppages          = $trip->route->stoppages;
        $source_pos         = array_search($request->pickup_point, $stoppages);
        ...
        // if(!empty($trip->day_off)) {
        //     if(in_array($dayOff, $trip->day_off)) {
        //         $notify[] = ['error', 'The trip is not available for '.$back_date->format('l')];
        //         return redirect()->back()->withNotify($notify);
        //     }
        // }

        $booked_ticket  = BookedTicket::where('trip_id', $id)
        ->where('date_of_journey', Carbon::parse($request->back_date)->format('Y-m-d'))
        ->whereIn('status',[1,2])
        ->where('pickup_point', $request->pickup_point)
        ->where('dropping_point', $request->dropping_point)
        ...
        foreach($seats as $seat){
                $parts = explode('-', $seat);
                $secondPart = $parts[1];       
                $seatBack = Seat::where(['ticket_price_id' => $ticketPrice->id,
                'name'=>$secondPart])->pluck('price')->first();
                $subTotalBack += $seatBack;
        }
        $seatsGoArray = $request->input('seats_go');

// Use array_filter to remove elements with a value equal to null
$filteredSeatsGoArray = array_filter($seatsGoArray, function ($value) {
    return $value !== null;
});

        foreach($filteredSeatsGoArray as $key=>$value){
            if($value!=null){
                        $parts = explode('-', $value);
                        $secondPart = $parts[1];       
                        $seatPriceGo = Seat::where(['ticket_price_id' => $ticketPrice->id,
                        'name'=>$secondPart])->pluck('price')->first();
                        $subTotalGo += $seatPriceGo;
            }
    }
    $subTotal = $subTotalBack + $subTotalGo;
        ...
        $bookedTicket->user_id = auth()->user()->id;
        $bookedTicket->gender = $request->gender;
        $bookedTicket->trip_id = $request->go_trip;
        $bookedTicket->source_destination = [$request->pickup_point, $request->dropping_point];
        $bookedTicket->pickup_point = $request->pickup_point;
        $bookedTicket->dropping_point = $request->dropping_point;
        $bookedTicket->seats_back = $seats;
        $bookedTicket->seats = $request->input('seats_go');

        $bookedTicket->ticket_count = sizeof($seats)+sizeof($request->input('seats_go'));
        // $bookedTicket->unit_price = $unitPrice;
        $bookedTicket->back_date = Carbon::parse($request->date_of_journey)->format('Y-m-d');
        $bookedTicket->date_of_journey = Carbon::parse($request->date_of_journey_go)->format('Y-m-d');
        $bookedTicket->pnr_number = $pnr_number;
        $bookedTicket->status = 0;
        $bookedTicket->sub_total = $subTotal;
        $bookedTicket->save();
    ...
    public function ticketSearch(Request $request)
    {
        ...
        if($request->go_date && Carbon::parse($request->go_date)->format('Y-m-d') < Carbon::now()->format('Y-m-d')){
            $notify[] = ['error', 'Date of journey can\'t be less than today.'];
            return redirect()->back()->withNotify($notify);
        }
       if($request->back_date && ( Carbon::parse($request->back_date)->format('Y-m-d') < Carbon::now()->format('Y-m-d')|| Carbon::parse($request->back_date)->format('Y-m-d') < Carbon::parse($request->go_date)->format('Y-m-d'))){
            $notify[] = ['error', 'Date of journey can\'t be less than today or Go Date.'];
            return redirect()->back()->withNotify($notify);
        }
        $trips = Trip::active();
        ...
        if($request->pickup && $request->destination){
            ...
            {
                $trips = $trips->where(['start_from'=>$request->pickup,'end_to'=>$request->destination])
                ->orWhere(['start_from'=>$request->destination,'end_to'=>$request->pickup])
                ->with('route')
                ->get();
            }
            else{
    ...
    public function getSeatPrice(Request $request)
    {
        ...
        // Implement logic to get seat price based on route, fleet type, and seat number
        $ticketPrice = TicketPrice::where(['fleet_type_id' => $fleetTypeId,
        'vehicle_route_id'=>$routeId])->first();
        $seatPrice = Seat::where(['ticket_price_id' => $ticketPrice->id,
        'name'=>$secondPart])->pluck('price')->first();
        return response()->json(['seatPrice' => $seatPrice]);
    }
